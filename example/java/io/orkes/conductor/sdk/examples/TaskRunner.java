...
/**
 * Task worker example.
 * Shows how you can create a worker that polls and executes a SIMPLE task in the conductor workflow.
 */
public class TaskRunner {
    ...
    /**
     * An example worker that is a long-running and periodically gets polled to check the status of a long-running task.
     * A use case for such worker is when the actual work is happening via separate process that could take longer to execute
     * (anywhere from minutes to days or months), e.g. database backup, running a background job that could take hours.
     * In such cases, such tasks are polled every so often (e.g. hourly or daily) to check the status and remains in progress
     * for much longer without holding up threads.
     */
