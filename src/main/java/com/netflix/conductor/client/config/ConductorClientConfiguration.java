...
public interface ConductorClientConfiguration {
   ...
   *     processed based on {@link ConductorClientConfiguration#isExternalPayloadStorageEnabled()}.
   */
  int getWorkflowInputPayloadThresholdKB();

  /**
   * @return the max value of workflow input payload size threshold in KB, beyond which the payload
  ...
  /**
   * @return the flag which controls the use of external storage for storing workflow/task input and
   *     output JSON payloads with size greater than threshold. If it is set to true, the payload is
   *     stored in external location. If it is set to false, the payload is rejected and the
   *     task/workflow execution fails.
   */
