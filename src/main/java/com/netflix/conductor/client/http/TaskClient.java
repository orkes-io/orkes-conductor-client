...
public abstract class TaskClient {
  ...
  /**
   * /** Updates the result of a task execution. If the size of the task output payload is bigger
   * than {@link ExternalPayloadStorage}, if enabled, else the task is marked as
   ...
   * @return true if the task was found with the given ID and acknowledged. False otherwise. If the
   *     server returns false, the client should NOT attempt to ack again.
   */
