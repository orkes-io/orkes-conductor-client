...
class TaskRunner {
  ...
  TaskRunner(
    ...
    LOGGER.info(
        "Polling errors will be sampled at every {} error (after the first 100 errors) for taskType {}",
        this.errorAt,
        taskType);
    ...
    LOGGER.info(
        ...
        domain);
    LOGGER.info(
        "Polling errors for taskType {} will be printed at every {} occurance.", taskType, errorAt);
  }
  ...
  private List<Task> pollTasksForWorker() {
    ...
    Boolean discoveryOverride =
        Optional.ofNullable(PropertyFactory.getBoolean(taskType, OVERRIDE_DISCOVERY, null))
    ...
    try {
      ...
      LOGGER.trace(
          "Polling task of type: {} in domain: '{}' with size {}", taskType, domain, pollCount);
  ...
  private void processTask(Task task) {
    LOGGER.trace(
        ...
        worker.getIdentity());
    LOGGER.trace(
        "task {} is getting executed after {} ms of getting polled",
        task.getTaskId(),
  ...
  private void executeTask(Worker worker, Task task) {
    ...
    try {
      ...
      LOGGER.error(
          "Unable to execute task: {} of type: {}", task.getTaskId(), task.getTaskDefName(), e);
