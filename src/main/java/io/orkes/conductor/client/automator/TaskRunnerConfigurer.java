...
public class TaskRunnerConfigurer {
  ...
  public static class Builder {
    ...
    private Map<String /* taskType */, Integer /* timeoutInMillisecond */> taskPollCount =
        new HashMap<>();
    ...
    /**
     * @param workerNamePrefix prefix to be used for worker names, defaults to workflow-worker- if
     *     not supplied.
    ...
    /**
     * @param eurekaClient Eureka client - used to identify if the server is in discovery or not.
     *     When the server goes out of discovery, the polling is terminated. If passed null,
     *     discovery check is not done.
     * @return Builder instance
     */
  ...
  public synchronized void init() {
    ...
    if (apiClient.isUseGRPC()) {
      LOGGER.info(
          "Using gRPC for task poll/update for ",
          workers.stream().map(worker -> worker.getTaskDefName()).collect(Collectors.toList()));
      workers.forEach(
          worker -> scheduledExecutorService.submit(() -> this.startPooledGRPCWorker(worker)));
    } else {
  ...
  private void startPooledGRPCWorker(Worker worker) {
    ...
    String domain =
        Optional.ofNullable(PropertyFactory.getString(taskType, DOMAIN, null))
            .orElseGet(
                () ->
                    Optional.ofNullable(PropertyFactory.getString(ALL_WORKERS, DOMAIN, null))
                        .orElse(taskToDomain.get(taskType)));
    LOGGER.info(
        "Starting gRPC worker: {} with {} threads", worker.getTaskDefName(), threadCountForTask);
